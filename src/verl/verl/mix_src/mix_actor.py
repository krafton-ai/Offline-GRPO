...
class MIXDataParallelPPOActor(DataParallelPPOActor):
    def __init__(
        ...
        self.adaptive_temperature_target_entropy = (
            self.config.adaptive_temperature_target_entropy
        )
        if self.use_adaptive_temperature:
            self.log_alpha = torch.tensor(
                np.log(self.config.entropy_coeff), dtype=torch.float
            )
    ...
    def update_policy(self, data: DataProto):
        ...
        if (
            self.config.use_off_policy_loss
            and self.config.off_policy_loss_impl == "seq"
        ):
        ...
        if self.config.use_off_policy_loss and self.config.use_off_policy_probs:
            ...
            # Check for multiple targets support
            if "target_probs_lst" in data.batch:
                select_keys.append("target_probs_lst")
            if "target_indices" in data.batch:
                select_keys.append("target_indices")
            ...
            for batch_idx, data in enumerate(dataloader):
                ...
                for data in micro_batches:
                    ...
                    if self.config.use_sft_multitask_loss:
                        assert (
                            self.config.use_off_policy_loss is False
                        ), "Either use off-policy loss or sft multitask loss. You cannot set both to be True."
                        ...
                        sft_loss = compute_sft_pure_loss(
                            log_prob=off_policy_logprob, eos_mask=off_policy_eos_mask
                        )
                        ...
                        # Prepare parameters for the loss function
                        loss_args = {
                            ...
                            "off_max_clip": (
                                self.config.off_policy_max_clip
                                if self.config.off_policy_max_clip != -1
                                else None
                            ),
                            "off_min_clip": (
                                self.config.off_policy_min_clip
                                if self.config.off_policy_min_clip != -1
                                else None
                            ),
                            "all_max_clip": (
                                self.config.all_max_clip
                                if self.config.all_max_clip != -1
                                else None
                            ),
                        ...
                        # Add target probs if available
                        if "target_probs" in data:
                            loss_args["target_probs"] = data["target_probs"]

                        # Add multiple targets support
                        if "target_probs_lst" in data:
                            loss_args["target_probs_lst"] = data["target_probs_lst"]
                        if "target_indices" in data:
                            loss_args["target_indices"] = data["target_indices"]

                        ret_dict = loss_fn(**loss_args)
                        ...
                        if "off_policy_prob" in ret_dict:
                            metric_data["actor/off_policy_prob"] = (
                                ret_dict["off_policy_prob"].detach().item()
                            )
                        if "on_policy_prob" in ret_dict:
                            metric_data["actor/on_policy_prob"] = (
                                ret_dict["on_policy_prob"].detach().item()
                            )
                        if "off_ratio_mean" in ret_dict:
                            metric_data["actor/off_ratio_mean"] = (
                                ret_dict["off_ratio_mean"].detach().item()
                            )
                        if "off_ratio_max_clip_frac" in ret_dict:
                            metric_data["actor/off_ratio_max_clip_frac"] = (
                                ret_dict["off_ratio_max_clip_frac"].detach().item()
                            )
                        if "off_ratio_min_clip_frac" in ret_dict:
                            metric_data["actor/off_ratio_min_clip_frac"] = (
                                ret_dict["off_ratio_min_clip_frac"].detach().item()
                            )
                    ...
                    if self.config.use_adaptive_temperature:
                        if self.config.use_adaptive_temperature_fixed is False:
                            ...
                            if self.config.adaptive_temperature_clip > 0:
                                entropy_coeff = torch.clamp(
                                    entropy_coeff,
                                    max=self.config.adaptive_temperature_clip,
                                )
                            alpha_loss = (
                                verl_F.masked_mean(
                                    entropy - target_entropy, response_mask
                                ).detach()
                                * entropy_coeff
                            )
                            ...
                            policy_loss = (
                                pg_loss - entropy_loss * entropy_coeff.detach().item()
                            )
                            metrics["actor/alpha_loss"] = alpha_loss.detach().item()
                            metrics["actor/entropy_coeff"] = (
                                entropy_coeff.detach().item()
                            )
                            ...
                            entropy_coeff = (
                                target_entropy / entropy_loss
                            ).detach().item() * self.config.entropy_coeff
                    ...
                    if self.config.use_ppo_kl_loss:
                        policy_loss = (
                            policy_loss + ppo_kl.abs() * self.config.ppo_kl_loss_coef
                        )
