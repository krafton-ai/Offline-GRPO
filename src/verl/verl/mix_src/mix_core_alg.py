...
def compute_token_on_off_policy_loss(
    ...
    target_probs=None,
    target_probs_lst=None,  # New parameter for multiple target probs
    target_indices=None,  # New parameter for selected target indices
    ...
    Args:
        ...
        prefix_mask: `(torch.Tensor)`
            shape: (bs, response_length)
        target_probs_lst: `(torch.Tensor)`
            shape: (bs, num_targets, response_length) - List of target probabilities for multiple targets
        target_indices: `(torch.Tensor)`
            shape: (bs,) - Indices of which target was used for each sample in the batch
    ...
    if on_policy_reshape == "no_reshape":
        ...
        ratio = (
            ...
            + on_policy_reshape_weight * negative_approx_kl
        )
    ...
    elif on_policy_reshape == "p_div_p_1.1":
        prob = torch.exp(log_prob)
        old_prob = torch.exp(old_log_prob)
        f_prob = 0.1 / (1.1 - prob)
        f_old_prob = 0.1 / (1.1 - old_prob)
        ratio = f_prob / f_old_prob
    elif on_policy_reshape == "p_div_p_0.01":
        prob = torch.exp(log_prob)
        old_prob = torch.exp(old_log_prob)
        f_prob = prob / (prob + 0.01)
        f_old_prob = old_prob / (old_prob + 0.01)
        ratio = f_prob / f_old_prob
    ...
    if loss_remove_clip is False:
        ...
        on_pg_clipfrac = verl_F.masked_mean(
            torch.gt(on_pg_losses2, on_pg_losses).float(), eos_mask
        )
    ...
    if target_probs is None and target_probs_lst is None:
        # No target probs provided, just use current log_prob
        ...
        elif off_policy_reshape == "p_div_p_1.1":
            off_ratio = 0.1 / (1.1 - off_ratio)
        ...
        if target_probs_lst is not None and target_indices is not None:
            # Handle multiple targets
            batch_size = log_prob.shape[0]
            seq_len = log_prob.shape[1]

            # Initialize with default shape
            effective_target_probs = torch.zeros_like(log_prob)

            # For each example in the batch, select the target probability based on target_indices
            for i in range(batch_size):
                target_idx = target_indices[i].item()
                if target_idx >= 0:  # If a valid target was used
                    effective_target_probs[i] = target_probs_lst[i, target_idx]

            # Use the selected target probabilities
            off_ratio = torch.exp(log_prob) / (effective_target_probs + 1e-6)
            off_ratio = off_ratio * prefix_mask
        else:
            # Original code for single target
    ...
    if off_max_clip is not None:
        off_ratio_ = torch.clamp(off_ratio, max=off_max_clip)
        off_ratio_max_clip_frac = verl_F.masked_mean(
            (off_ratio_ == off_max_clip).float(), prefix_mask * eos_mask
        )
    else:
        off_ratio_max_clip_frac = torch.tensor(0.0)

    if off_min_clip is not None:
        off_ratio_ = torch.clamp(off_ratio, min=off_min_clip)
        off_ratio_min_clip_frac = verl_F.masked_mean(
            (off_ratio_ == off_min_clip).float(), prefix_mask * eos_mask
        )
    else:
    ...
    if loss_remove_clip is False:
        upper_bound = max(clip_upper_bound, 1.0 + off_cliprange)
        if upper_bound == clip_upper_bound:
            print("clip upper bound is used: ", clip_upper_bound)
        off_pg_losses2 = -advantages * torch.clamp(
            off_ratio, 1.0 - off_cliprange, upper_bound
        )
        off_pg_clipfrac = verl_F.masked_mean(
            torch.gt(off_pg_losses2, off_pg_losses).float(), eos_mask
        )
